...
async function testBiontechVaccine() {
    ...
    // Choose Biontech 
    const biontechButton = await client.$("(//*[contains(@class,'RadioButton')])[1]")
    ...
    const vaccineTypeField = await client.$("(//*[contains(@class,'TextView')])[6]")
    const vaccineType = await vaccineTypeField.getAttribute("text")

    console.log(vaccineType)
    await client.setImplicitTimeout(3000);

    await client.deleteSession()
    return assert.strictEqual(vaccineType, "Vaccine Type: Biontech")
}

async function testSinovacVaccine() {
    const client = await wdio.remote(opts)
    ...
    // Check if Vaccine Type is Listed as Biontech
    const vaccineTypeField = await client.$("(//*[contains(@class,'TextView')])[6]")
    const vaccineType = await vaccineTypeField.text

    console.log(vaccineType)
    await client.setImplicitTimeout(3000);

    await client.deleteSession()
    return assert.strictEqual(vaccineType, "Vaccine Type: Sinovac")
}

module.exports = { testBiontechVaccine, testSinovacVaccine }